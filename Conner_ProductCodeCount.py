...
df = pd.read_excel (r'Product_Code_Test.xlsx')
...
for num in product_code:
    requested_url = 'https://api.fda.gov/device/registrationlisting.json?search=products.product_code:' + 'NIK' + '&limit=1&api_key=lRpQPBNqJQdhH5ftVdMsURt9a96IHLeYNPG5cwFt'
    name = product_code[ind]
    try:
        res = requests.get(requested_url)
        data = res.json()
        pcode_count =  data.results['proprietary_name']
        print(pcode_count)
...
final2.to_excel("NIK_Establishment_PCODE_COUNT.xlsx")
